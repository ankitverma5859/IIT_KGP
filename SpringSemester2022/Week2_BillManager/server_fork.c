...
{
    printf("Data files should be present in data/*.files at client side.\nServer side should have server_data/ director.\nSample Files:\n\tSorting: sort.txt\n\tMerge: merge_1.txt merge_2.txt\n\tSimilarity: sim_1.txt sim_2.txt\n");
            ...
            *client_count = *client_count + 1;
            ...
            if((child_pid = fork()) == 0)
            {
                ...
                while(1)
                {
                    ...
                    read_write_error_check(n, client_count, newsock_fd);
                    ...
                    {
                        printf("Client %s:%d\t:\t %s\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port), message_from_client);
                        ...
                        else
                        {
                            if(strcmp(command_to_execute, SORT) == 0)
                            {
                                ...
                                n = read(newsock_fd, data, filesize);
                                ...
                                if(validate_file(file_location))
                                {
                                    ...
                                    n = write(newsock_fd, fsize, strlen(fsize));
                                    
                                    //Receive request for file
                                    n = read(newsock_fd, message_from_client, BUFFER_SIZE);
                                    
                                    //Send the sorted content to client
                                    n = write(newsock_fd, data, filesize);
                            ...
                            else if(strcmp(command_to_execute, MERGE) == 0)
                            {
                                ...
                                n = read(newsock_fd, data1, filesize_1);
                                read_write_error_check(n, client_count, newsock_fd);
                                ...
                                n = read(newsock_fd, data2, filesize_2);
                                read_write_error_check(n, client_count, newsock_fd);
                            ...
                            else if(strcmp(command_to_execute, SIMILARITY) == 0)
                            {
                                ...
                                n = read(newsock_fd, data, filesize_1);
                                read_write_error_check(n, client_count, newsock_fd);
                                ...
                                if(validate_file(file_location_1))
                                {
                                    ...
                                    n = read(newsock_fd, data, filesize_2);
                                    read_write_error_check(n, client_count, newsock_fd);
